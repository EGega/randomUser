 ...
 <div className={userStyle.wrapper}>
      ...
      <div id="displayer-div" className={userStyle.displayer}>
        ...
        <div className={userStyle.icons}>
         {user[0].gender === "female" ? <img src={femaleUser} alt="" className={userStyle.avatar} onMouseEnter={() => {
          setHoveredElement({...hoveredElement, name:true})}} onMouseLeave={() => {
            setHoveredElement({...hoveredElement, name:false}) 
           }}  /> 
         : <img className={userStyle.avatar} src={maleUser} alt=""  onMouseEnter={() => {
          setHoveredElement({...hoveredElement, name:true})}} onMouseLeave={() => {
            setHoveredElement({...hoveredElement, name:false}) 
           }} />} 
         <img src={userMail} onMouseEnter={() => {
          setHoveredElement({...hoveredElement, email:true})}} onMouseLeave={() => {
            setHoveredElement({...hoveredElement, email:false}) 
           }}  alt="" />
         {user[0].gender === "female" ? <img src={womanAge} onMouseEnter={() => {
          setHoveredElement({...hoveredElement, age:true})}} onMouseLeave={() => {
            setHoveredElement({...hoveredElement, age:false}) 
           }}  alt="" /> : <img src={manAge} alt="" onMouseEnter={() => {
            setHoveredElement({...hoveredElement, age:true})}} onMouseLeave={() => {
              setHoveredElement({...hoveredElement, age:false}) 
             }}  />}
         <img src={userLocation} onMouseEnter={() => {
          setHoveredElement({...hoveredElement, location:true})}} onMouseLeave={() => {
            setHoveredElement({...hoveredElement, location:false}) 
           }}  alt="" />
         <img src={userPhone}  onMouseEnter={() => {
          setHoveredElement({...hoveredElement, phone:true})}} onMouseLeave={() => {
            setHoveredElement({...hoveredElement, phone:false}) 
           }}  alt="" />
         <img src={userPassword}  onMouseEnter={() => {
          setHoveredElement({...hoveredElement, login:true})}} onMouseLeave={() => {
            setHoveredElement({...hoveredElement, login:false}) 
           }}  alt="" />
        </div>
        <Button submit={submit} /> 
        <button onClick={displayHandler} className={userStyle.btn}>Add User</button>
        <UserDisplay displayHandler={displayHandler} addedUser={addedUser}/>
